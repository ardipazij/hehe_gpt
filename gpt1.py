...
'''
    ...
    def forward(self, x):
            ...
            return logits
    def generate(self, x : torch.Tensor, max_new_tokens : int, do_sample : bool, top_k : int, top_p : float,temperature = 1.0):
        '''
        ...
        for _ in range(max_new_tokens):
            ...
            if do_sample:
                ...
                if top_p is not None:
                    ...
                    keep_sorted[:, 0] = 1 # Всегда оставляем токен с мин. кум. вероятностью
                    
                    top_p_mask = torch.zeros_like(keep_sorted, dtype=torch.uint8)
                    top_p_mask.scatter_(dim=-1, index=sorted_indices, src=keep_sorted.byte())
        ...
        for epoch in range(num_epoch):
            ...
            with torch.no_grad():
                for inputs, targets in valid_loader:
                    ...
                    v_loss = torch.nn.functional.cross_entropy(logits_flat, targets_flat)
